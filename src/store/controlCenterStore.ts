        ...
        switch (transportEvent.type) {
          case 'transport.status':
            set({
              ...
              ...(transportEvent.status !== 'connected' ? { stale: true } : {}),
            });
            return;
          ...
          case 'ai.session.deleted':
            ...
            return;
	          ...
	          case 'ai.sessions.updated':
	            if (get().serverMode) {
	              scheduleRefreshDebounce(() => get().refreshFromServer().catch(() => {}));
            }
            return;
            ...
            set(state => ({
              terminalSessions: state.terminalSessions.map(ts =>
                ts.id === transportEvent.sessionId
                  ? {
                      ...ts,
                      lines: tail([...ts.lines, line('stdout', transportEvent.data)], MAX_TERMINAL_LINES),
                      updatedAt: nowTime(),
                    }
                  : ts
              ),
            ...
            set(state => ({
              ...
              vibeRuns: state.vibeRuns.map(run =>
                run.id === preview.sessionId
                  ? { ...run, status: 'preview_ready' as VibeStatus, previewId: preview.id, lastActivityMs: activityNowMs(), updatedAt: formatActivityLabel(activityNowMs()) }
                  : run
              ),
              events: [
